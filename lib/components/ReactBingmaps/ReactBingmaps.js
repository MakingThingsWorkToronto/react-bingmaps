			...
			if(data && data.length> 0) {
				...
				for(var i = 0; i<data.length; i++){
					...
					var polygon = new Microsoft.Maps.Polygon(locs, {
						fillColor: row.fillColor || "#f4f4f4",
						strokeColor: row.strokeColor || "#a4262c",
						strokeThickness: row.strokeColor || 5,
						
					});
				
					mapReference.entities.push(polygon);
				...
				mapReference.setView({ bounds: Microsoft.Maps.LocationRect.fromLocations(locs) });
