...
var ReactBingmaps = function (_Component) {
	...
	_createClass(ReactBingmaps, [{
		...
		value: function componentWillReceiveProps(nextProps) {
			...
			if ((this.props.center && nextProps.center && this.props.center.join() !== nextProps.center.join()) || (!this.props.center && nextProps.center)) {
			...
			if(nextProps.compressedPolygons && this.props.compressedPolygons && nextProps.compressedPolygons.length > 0 && this.props.compressedPolygons !== nextProps.compressedPolygons) {
				this.renderCompressedPolygons(nextProps.compressedPolygons, mapReference);
			}
		}
	}, {
		key: 'reactBingmaps',
		value: function reactBingmaps(props, Microsoft) {
			var bingmapKey = props.bingmapKey,
				...
				compressedPolygons = props.compressedPolygons;

			if (bingmapKey && Microsoft) {
				var mapReference = props.id ? '#' + props.id : '.react-bingmaps';
				if (!map[mapReference]) {
					var so = startOptions || {};
					so.credentials = bingmapKey;
					map[mapReference] = new Microsoft.Maps.Map(mapReference, so);
				}
				...
				this.setMapBounds(bounds, mapReference, mapOptions);
				...
				this.renderCompressedPolygons(compressedPolygons, mapReference);
			}
		}
	}, {
		key: 'setMapCenter',
		...
		key: 'setMapBounds',
		value: function setMapBounds(bounds, mapReference, mapOptions) {

			if (map[mapReference] && bounds && bounds.length > 0) {
				
				var locations = [],
					zoom;
				
				for(var ii = 0; ii<bounds.length; ii++){
					locations.push(new Microsoft.Maps.Location(bounds[ii][0],bounds[ii][1]));
				}
				...
				map[mapReference].setView({
					...
					padding: 0 //Add a padding to buffer map to account for pushpin pixel dimensions
				});

				zoom = map[mapReference].getZoom();

				if(mapOptions && typeof mapOptions.maxZoom == "number" && mapOptions.maxZoom >= 1 && mapOptions.maxZoom <= 19 
					&& zoom > mapOptions.maxZoom) {
					map.setMapZoom(mapOptions.maxZoom, mapReference);
				}
				
				if(mapOptions && typeof mapOptions.minZoom == "number" && mapOptions.minZoom >= 1 && mapOptions.minZoom <= 19 
					&& zoom < mapOptions.minZoom) {
					map.setMapZoom(mapOptions.minZoom, mapReference);
				}
				...
				if (infoboxesWithPushPins) {
					for (var infoboxWithPushPinIndex = 0; infoboxWithPushPinIndex < infoboxesWithPushPins.length; infoboxWithPushPinIndex++) {
						if (infoboxesWithPushPins[infoboxWithPushPinIndex].location
							&& !isNaN(infoboxesWithPushPins[infoboxWithPushPinIndex].location[0])
							&& !isNaN(infoboxesWithPushPins[infoboxWithPushPinIndex].location[1])
							) {
							...
							try {
								...
								if (infoboxOption.htmlContent && typeof infoboxOption.htmlContent != "string") {
								...
								console.log("Failure adding pushpin: " + infoboxesWithPushPins[infoboxWithPushPinIndex].location);
								console.log(ex);
		...
		key: 'renderCompressedPolygons',
		value: function renderCompressedPolygons(data, mapReference) {
			
			if(map[mapReference] && data && data.length> 0) {
				...
				Microsoft.Maps.loadModule('Microsoft.Maps.SpatialMath', function () {
					...
					for(var i = 0; i<data.length; i++){
						...
						var row = data[i];
						var title = row.title;
						var encodedString = row.shape;
						
						if(encodedString && encodedString.length > 2) {

							try {

								console.log("Trying to render polygon: " + encodedString);
								
								var loc = Microsoft.Maps.PointCompression.decode(encodedString.substr(2));
								
								//Display the locations as a polyline on the map so we can see what it looks like.
								var polygon = new Microsoft.Maps.Polygon(loc, {
									fillColor: row.fillColor || "rgba(244,244,244,0.5)",
									strokeColor: row.strokeColor || "#a4262c",
									strokeThickness: row.strokeThickness || 2
								});
								...
								if(row.target && row.target.length > 0) {
									...
									//Add a click event to the polygon which will navigate to a different page
									Microsoft.Maps.Events.addHandler(polygon, 'click',  (function(target){
										return function (e) { 
								...
								if(title && title.length > 0){
									...
									map[mapReference].entities.push(labelPin);
								...
								console.log("Failure adding polygon");
								console.log(ex);
				...
				//if(locs && locs.length>0) map[mapReference].setView({ bounds: Microsoft.Maps.LocationRect.fromLocations(locs) });
...
ReactBingmaps.propTypes = {
	...
	compressedPolygons: _propTypes2.default.arrayOf(_propTypes2.default.shape({
		title: _propTypes2.default.string,
		fillColor: _propTypes2.default.string,
		strokeColor: _propTypes2.default.string,
		strokeThickness: _propTypes2.default.number,
